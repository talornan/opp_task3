...
public class DirectedWeightedGraphAlgorithmslmpl implements DirectedWeightedGraphAlgorithms{
    ...
    public NodeData center() {
       ...
       for(NodeData node : graph.getVertices().values()){
           ...
           for(NodeData n : graph.getVertices().values()){
                dis += shortestPathDist(node.getKey(),n.getKey());
           }
    ...
    public boolean load(String file) {
        ...
        // set src from status as 0
        src.setFrom(0);
        // define PriorityQueue and add src there.
        ...
        for(NodeData  node : graph.getVertices().values()){
            ...
            ((NodeDataImpl) node).setVisit(false);
            // check if node different from src add node to priorityQueue
            if(node!= src)
            priorityQueue.add((NodeDataImpl) node);
        }
        // update src Weight status to 0
        src.setWeight(0);
...
// this function is to base on DFS algo
